...
void main() {
  ...
  group('BatchUploadTalks', () {
    ...
    test('should call repository.batchUploadTalks with correct parameters', () async {
      when(() => mockRepository.batchUploadTalks(eventId, testTalks))
          .thenAnswer((_) async => const Right(BatchUploadResult(
                createdCount: 2,
                updatedCount: 0,
                skippedRows: [],
              )));
      ...
      verify(() => mockRepository.batchUploadTalks(eventId, testTalks)).called(1);
    });

    test('should return BatchUploadResult with created counts on success', () async {
      when(() => mockRepository.batchUploadTalks(eventId, testTalks))
          .thenAnswer((_) async => const Right(BatchUploadResult(
                createdCount: 2,
                updatedCount: 0,
    ...
    test('should return BatchUploadResult with skipped rows', () async {
      when(() => mockRepository.batchUploadTalks(eventId, testTalks))
          .thenAnswer((_) async => const Right(BatchUploadResult(
                ...
                updatedCount: 0,
                skippedRows: [
                  SkippedRow(rowNumber: 3, reason: 'Invalid data'),
                ],
              )));
    ...
    test('should handle empty talks list', () async {
      when(() => mockRepository.batchUploadTalks(eventId, any()))
          .thenAnswer((_) async => const Right(BatchUploadResult(
                createdCount: 0,
                updatedCount: 0,
                skippedRows: [],
